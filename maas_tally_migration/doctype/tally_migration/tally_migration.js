...
frappe.ui.form.on("Tally Migration", {
            ...
            if (data.count == data.total || error_occurred) {
                window.setTimeout(
                    (title) => {
                        ...
                        if (!error_occurred) {
                            frappe.show_alert({
                                message: __("{0} completed successfully", [data.title]),
                                indicator: "green",
                            });
                        }
                        if (error_occurred) {
    ...
    refresh: function (frm) {
        ...
        if (
            frm.doc.is_master_data_imported
            && !frm.doc.is_opening_balances_imported
            && (frm.doc.opening_sales_invoices || frm.doc.opening_purchase_invoices || frm.doc.opening_stock_items)
        ) {
            if (frm.doc.status != "Importing Opening Balances") {
                frm.events.add_button(frm, __("Import Opening Balances"), "_import_opening_balances");
            }
        }
        ...
        if (
            frm.doc.is_master_data_imported
            && frm.doc.ledger_opening_balances
            && frm.doc.opening_posting_date
            && !frm.doc.is_opening_journal_entry_imported
        ) {
            if (frm.doc.status != "Importing Opening Journal Entry") {
                frm.events.add_button(frm, __("Import Opening Journal Entry"), "_import_opening_journal_entry");
            }
        }
